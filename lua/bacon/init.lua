...
function Bacon.bacon_load()
  ...
  -- Check if the current working directory matches one in project_dirs
  for _, dir in pairs(config.options.project_dirs) do
    if cwd == dir.project_dir then
      rust_dir = dir.rust_dir
      break
    end
  end

  local dir = rust_dir -- Start with the rust_dir if it's set
